...
def flesch_kincaid_grade(text: str) -> float:
    ...
    In some environments, textstat's English readability calculation depends on
    the NLTK cmudict corpus. If that corpus has not been downloaded yet, raise
    a clear setup message instead of a long traceback.
    """
    try:
        return textstat.flesch_kincaid_grade(text)
    except LookupError as exc:
        raise SystemExit(
            "textstat readability requires the NLTK cmudict corpus in this environment. "
